...
export default function CardBoxTemperature() {
    ...
    function getStateTemp1(temperature: number) {
        if (temperature > 40) {
            return { color: "#ED1C24", message: "Attention, la température ambiante est trop élevée !", alert : 'error'} as
                { color: string; message: string; alert: 'error' | 'warning' | 'info' | 'success'}
        } else if (temperature > 30) {
            return { color: "#FFC20A", message: "Attention, la température ambiante est legèrement trop élevée !", alert : 'warning'} as
                { color: string; message: string; alert: 'error' | 'warning' | 'info' | 'success'}
        } else {
            return { color: "#22B04B", message: "La température ambiante est bonne !", alert : 'success'} as
                { color: string; message: string; alert: 'error' | 'warning' | 'info' | 'success'}
        }
    }
    ...
    function getStateTemp2(temperature: number) {
        if (temperature > 10) {
            return { color: "#ED1C24", message: "Attention, la bière est trop chaude !", alert : 'error'} as
                { color: string; message: string; alert: 'error' | 'warning' | 'info' | 'success'}
        } else if (temperature >= 7) {
            return { color: "#FFC20A", message: "Attention, la bière est légèrement trop chaude !", alert : 'warning'} as
                { color: string; message: string; alert: 'error' | 'warning' | 'info' | 'success'}
        } else if (temperature >= -2) {
            return { color: "#22B04B", message: "La bière est à bonne température !", alert : 'success'} as
                { color: string; message: string; alert: 'error' | 'warning' | 'info' | 'success'}
        } else {
            return { color: "#ED1C24", message: "Attention, la bière est trop froide !", alert : 'error' } as
                { color: string; message: string; alert: 'error' | 'warning' | 'info' | 'success'}
        }
    }
